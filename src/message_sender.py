...
from src.contact_verifier import ContactVerifyResult, ocr_from_path, verify_active_contact
...
SAFE_TEST_CONTACT = "File Transfer"
_SAFE_TEST_CONTACT_ALIASES = {"File Transfer", "文件传输助手"}


def _allowed_real_contacts(config: dict[str, Any]) -> list[str]:
    """Return the list of contacts permitted for real sending.

    Always includes both File Transfer and 文件传输助手 aliases.
    Additional contacts are read from config['allowed_real_contacts'].
    """
    base: list[str] = list(_SAFE_TEST_CONTACT_ALIASES)
...
def send_message(
    ...
    *,
    ...
    verify_contact_func: Callable[
        [dict[str, Any], str, Callable, Callable], ContactVerifyResult
    ] = verify_active_contact,
    ocr_func: Callable[[str, dict[str, Any]], list[dict[str, Any]]] = ocr_from_path,
) -> bool:
    ...
    for attempt in range(1, max_retry + 1):
        ...
        try:
            ...
            # OCR-verify the correct contact chat is now open before sending.
            verify_result = verify_contact_func(
                config, target, screenshot_func, ocr_func
            )
            if not verify_result.ok:
                raise RuntimeError(
                    f"Contact OCR verification failed: {verify_result.message}"
                )
            LOGGER.info(
                "Contact verified via OCR: target='%s' screenshot=%s",
                target,
