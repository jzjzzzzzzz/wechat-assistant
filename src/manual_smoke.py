...
SAFE_MANUAL_TEST_CONTACTS = {SAFE_TEST_CONTACT, "文件传输助手"}
...
def validate_safe_manual_config(config: dict[str, Any]) -> ManualSmokeResult:
    ...
    if config.get("test_contact") not in SAFE_MANUAL_TEST_CONTACTS:
        allowed = ", ".join(sorted(SAFE_MANUAL_TEST_CONTACTS))
        return ManualSmokeResult("config-safety", False, f"test_contact must be one of: {allowed}.")
