...
def test_message_sender_allowed_file_transfer_helper_real_send_sequence_is_mocked(tmp_path: Path) -> None:
    ...
    result = send_message(
        ...
        verify_contact_func=lambda cfg, target, ss_fn, ocr_fn: calls.append("verify")
        or __import__("src.contact_verifier", fromlist=["ContactVerifyResult"]).ContactVerifyResult(
        ok=True, target=target, sidebar=None, title_bar=None, message="verified"
        ),
    )
