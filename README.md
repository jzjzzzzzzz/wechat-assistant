...
After contact search, the sender captures a fresh screenshot and OCR-checks both:

- the selected row in the left chat list
- the chat title above the message history

Both regions must match the target contact name or a known alias. This prevents
the common failure mode where automation searches or activates the wrong chat
after a restart, wake-up, or focus change.
...
If OCR cannot confirm both regions, the command blocks before paste/Enter and
